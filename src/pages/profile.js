...
class ProfilePage {
            ...
            <div class="page active" id="profile-page">
                ...
                <div class="profile-header">
                    <div class="profile-avatar">
                        ...
                        <div class="avatar-edit" onclick="profilePage.showAvatarUpload()">
                            <i class="fas fa-camera"></i>
                        </div>
                    </div>
                    <div class="profile-name" id="profile-nickname">未登录</div>
                    <div class="profile-contact" id="profile-contact">--</div>
                    <div class="profile-join-date" id="profile-join-date">加入时间：--</div>
                </div>
    ...
    updateData() {
        ...
        try {
            ...
            if (nameEl) nameEl.textContent = this.userData.nickname || '未登录';
            if (contactEl) {
                if (this.userData.phone) {
                    contactEl.textContent = this.userData.phone;
                } else if (this.userData.email) {
                    contactEl.textContent = this.userData.email;
                } else if (this.userData.provider && this.userData.provider !== 'local') {
                    contactEl.textContent = `第三方：${this.userData.provider}`;
                } else {
            ...
            if (joinDateEl) {
                ...
                const formattedDate = joinDate.toLocaleDateString('zh-CN', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                });
                joinDateEl.textContent = `加入时间：${formattedDate}`;
            }
        } catch (e) {}
    ...
    loadUserData() {
        try {
            ...
            const appData = JSON.parse(localStorage.getItem('accounting_app_data') || '{}');
            
            this.userData = {
                nickname: authUser.nickname || authUser.username || '用户',
            ...
            if (appData.userProfile) {
                this.userData = { ...this.userData, ...appData.userProfile };
            }
        } catch (error) {
        ...
        try {
            ...
            this.showModal('账户设置', `
                <div style="padding: 20px;">
                    ...
                    <div class="input-group">
                        <label>电子邮箱</label>
                        <input type="email" id="profile-email" value="${user?.email || ''}" placeholder="请输入电子邮箱">
                    </div>
                    ...
                    <div class="security-level high">
                        ...
                        <div>
                            <div style="font-weight: 600;">账户安全</div>
                            <div style="font-size: 0.8rem; opacity: 0.8;">您的账户安全等级：高</div>
                        </div>
                    </div>
                    
                    <!-- 密码修改 -->
                    <div style="margin-top: 20px; border-top: 1px solid #e2e8f0; padding-top: 20px;">
                        <h4 style="margin-bottom: 15px;">密码修改</h4>
                        <div class="input-group">
                            <label>当前密码</label>
                        ...
                        <div class="input-group">
                            <label>新密码</label>
                            <input type="password" id="new-password" placeholder="请输入新密码（最少8位）">
                        </div>
                        <div class="input-group">
                            <label>确认新密码</label>
                            <input type="password" id="confirm-password" placeholder="请再次输入新密码">
                        </div>
                        <button class="btn btn-secondary" onclick="profilePage.changePassword()" style="width: 100%; margin-top: 10px;">
                            <i class="fas fa-key"></i> 修改密码
                        </button>
                    </div>
                    
                    <div class="button-group" style="margin-top: 20px;">
                        <button class="btn btn-primary" onclick="profilePage.saveSettings()">保存个人信息</button>
                        <button class="btn btn-secondary" onclick="profilePage.hideModal()">取消</button>
                    </div>
                </div>
            `);
    ...
    showSettingsFallback() {
        ...
        this.showModal('账户设置', `
            <div style="padding: 20px;">
                ...
                <!-- 安全设置区域 -->
                <div class="security-level ${user?.phone || user?.email ? 'high' : 'medium'}">
                    <div class="security-icon"><i class="fas fa-shield-alt"></i></div>
                    <div>
                        <div style="font-weight: 600;">账户安全</div>
                        <div style="font-size: 0.8rem; opacity: 0.8;">您的账户安全等级：${user?.phone || user?.email ? '高' : '中'}</div>
                    </div>
                </div>
    ...
    async changePassword() {
        ...
        if (!currentPassword) {
            this.showAlert('请输入当前密码');
            return;
        }
        
        if (newPassword.length < 8) {
            this.showAlert('新密码长度不能少于8位');
            return;
        }
        
        if (newPassword !== confirmPassword) {
            this.showAlert('两次输入的新密码不一致');
            return;
        }
    ...
    showAvatarUpload() {
        this.showModal('上传头像', `
            <div style="padding: 20px; text-align: center;">
                <div style="margin-bottom: 20px;">
                    <div class="profile-avatar" style="margin: 0 auto 20px; position: relative; width: 120px; height: 120px;">
                        <i class="fas fa-user" style="font-size: 3rem;"></i>
                    </div>
                    <p style="color: #718096; font-size: 0.9rem;">支持 JPG、PNG 格式，文件大小不超过 2MB</p>
                </div>
                
                <div style="display: flex; gap: 10px; justify-content: center; margin-bottom: 20px;">
                    <button class="btn btn-primary" onclick="profilePage.selectAvatarFromGallery()">
                        <i class="fas fa-images"></i> 从相册选择
                    </button>
                    <button class="btn btn-secondary" onclick="profilePage.takePhoto()">
                        <i class="fas fa-camera"></i> 拍照
                    </button>
                </div>
                
                <!-- 文件上传区域 -->
                <div style="border: 2px dashed #e2e8f0; border-radius: 8px; padding: 30px; margin-bottom: 20px;">
                    <i class="fas fa-cloud-upload-alt" style="font-size: 2rem; color: #a0aec0; margin-bottom: 10px;"></i>
                    <p>拖拽图片到此处或点击上传</p>
                    <input type="file" id="avatar-upload" accept="image/*" style="display: none;" onchange="profilePage.handleAvatarUpload(this.files)">
    ...
    selectAvatarFromGallery() {
        // 模拟从相册选择
        this.showAlert('此功能暂未实现，请使用文件上传');
    }
    ...
    logout() {
        this.showModal('退出登录', `
            <div style="padding: 20px;">
                ...
                <!-- 退出选项 -->
                <div style="margin-bottom: 20px;">
                    <h4 style="margin-bottom: 10px;">退出选项</h4>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <label style="display: flex; align-items: center; gap: 8px;">
                            <input type="checkbox" id="clear-session" checked>
                            <span>清除本地会话数据</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <label style="display: flex; align-items: center; gap: 8px;">
                            <input type="checkbox" id="save-backup" checked>
                            <span>退出前备份数据</span>
                        </label>
                    </div>
                </div>
    ...
    async confirmLogout() {
        try {
            const clearSession = document.getElementById('clear-session')?.checked !== false;
            const clearCache = document.getElementById('clear-cache')?.checked || false;
            const saveBackup = document.getElementById('save-backup')?.checked !== false;
            
            // 如果有备份选项，执行备份
            if (saveBackup) {
                await this.performLogoutBackup();
            }
            ...
            if (clearCache) {
                // 清除应用缓存数据
                localStorage.removeItem('accounting_app_data');
                ...
                if ('indexedDB' in window) {
                    try {
                        ...
                        dbs.forEach(db => {
                            if (db.name && db.name.includes('accounting')) {
                                indexedDB.deleteDatabase(db.name);
                            }
                        });
                    } catch (error) {
                        console.warn('清除索引DB缓存失败:', error);
                    }
                }
            }
            
            // 显示退出成功消息
            this.app.showToast('已安全退出登录', 'success');
            ...
            this.app.showToast('退出登录失败，请重试', 'error');
        }
    }

    // 退出前执行备份
    async performLogoutBackup() {
        try {
            // 获取所有需要备份的数据
            const backupData = {
                timestamp: new Date().toISOString(),
                user: JSON.parse(localStorage.getItem('auth_user') || '{}'),
                transactions: JSON.parse(localStorage.getItem('accounting_app_data') || '{}'),
                settings: {
                    userMode: localStorage.getItem('user_mode'),
                    studentSettings: JSON.parse(localStorage.getItem('student_mode_settings') || '{}'),
                    familySettings: JSON.parse(localStorage.getItem('family_mode_settings') || '{}'),
                    freelancerSettings: JSON.parse(localStorage.getItem('freelancer_mode_settings') || '{}')
                }
            };
            ...
            a.href = url;
            a.download = `accounting_backup_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
            
            // 触发下载（在真实环境中会下载文件）
            console.log('备份数据已准备就绪:', backupData);
            
            // 清理URL
            URL.revokeObjectURL(url);
            
        } catch (error) {
            console.warn('备份数据失败:', error);
        }
    }
    ...
    // 增强隐私设置功能
    showEnhancedPrivacy() {
        this.showModal('隐私与数据管理', `
            <div style="padding: 20px;">
                <div style="margin-bottom: 20px;">
                    <h4 style="margin-bottom: 10px;">隐私设置</h4>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <p style="font-size: 0.8rem; color: #718096;">允许导出个人数据到文件</p>
                    </div>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <p style="font-size: 0.8rem; color: #718096;">允许匿名数据用于功能改进</p>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <!-- 数据清除选项 -->
                    <div style="margin-bottom: 15px;">
                        <button class="btn btn-outline" style="width: 100%; margin-bottom: 10px;" onclick="profilePage.clearAllData()">
                            <i class="fas fa-trash-alt"></i> 清除所有数据
                        </button>
                        <p style="font-size: 0.8rem; color: #718096;">永久删除所有本地存储数据</p>
                    </div>
                    
                    <!-- 数据导出 -->
                    <div style="margin-bottom: 15px;">
                        <button class="btn btn-outline" style="width: 100%; margin-bottom: 10px;" onclick="profilePage.exportAllData()">
                            <i class="fas fa-download"></i> 导出所有数据
                        </button>
                        <p style="font-size: 0.8rem; color: #718096;">下载完整的数据备份文件</p>
                    </div>
                </div>
    ...
    // 清除所有数据
    async clearAllData() {
        const confirmClear = confirm('确定要清除所有数据吗？此操作不可恢复！');
        if (confirmClear) {
            try {
                // 清除所有本地存储数据
                localStorage.clear();
                ...
                this.showAlert('所有数据已成功清除', 'success');
                this.hideModal();
                ...
                this.showAlert('清除数据失败，请重试', 'error');
            }
        }
    }
    ...
    exportAllData() {
        try {
            ...
            const exportData = {
                ...
                user: JSON.parse(localStorage.getItem('auth_user') || '{}'),
                appData: JSON.parse(localStorage.getItem('accounting_app_data') || '{}'),
                settings: {
                    userMode: localStorage.getItem('user_mode'),
                    studentMode: JSON.parse(localStorage.getItem('student_mode_settings') || '{}'),
                    familyMode: JSON.parse(localStorage.getItem('family_mode_settings') || '{}'),
                    freelancerMode: JSON.parse(localStorage.getItem('freelancer_mode_settings') || '{}')
                },
                transactionHistory: this.app?.transactions || []
            };
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `accounting_data_export_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
            ...
            this.showAlert('数据导出失败，请重试', 'error');
        }
    }
}
